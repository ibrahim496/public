...
<section class="right">
    ...
    if (isset($_SESSION['loggedin']) && $_SESSION['loggedin'] == true) {
        if (isset($_POST['submit'])) {
            $db = getConnection();
            $stmt = $db->prepare('UPDATE client SET username = :username, password = :password WHERE id = :id ');
            ...
            $db = getConnection();
            $currentAdmin = $db->query("SELECT * FROM client WHERE username = '" . $_GET['username'] . "'")->fetch();
